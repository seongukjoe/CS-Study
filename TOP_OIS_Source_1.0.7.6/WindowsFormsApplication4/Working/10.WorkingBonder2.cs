...
{
    public class WorkingBonder2JetData : BaseData
    {
        private string section;
        public List<JettingData> JetData;   // Point

        private int JetDataCount;
        public WorkingBonder2JetData(string name) : base(cDefString.Bonder2Point)
        {
        ...
        public override void DataProc(fpIni.IniType type)
        {
            ...
            {
                section = cDefString.Bonder2Point;
                bool enable = false;
                double radius = 0.0;
                double angle = 0.0;
                int delay = 0;
    ...
    public class WorkingBonder2JetLine : BaseData
    {
        private string section;
        public List<JettingLineData> JetLineData;  // Line

        private int JetLineDataCount;
        public WorkingBonder2JetLine(string name) : base(cDefString.Bonder2Line)
        {
        ...
        public override void DataProc(fpIni.IniType type)
        {
            if (type == fpIni.IniType.Read)
            {
                section = cDefString.Bonder2Line;
                // Line
                int xpos = 0;
                int ypos = 0;
                int zpos = 0;
                ...
                ini.DataProc(type, section, "JetLineDataCount", ref JetLineDataCount);
                JetLineData.Clear();
                for (int i = 0; i < JetLineDataCount; i++)
                {
                    ini.DataProc(type, section, $"{i}_XPos", ref xpos);
                    ini.DataProc(type, section, $"{i}_YPos", ref ypos);
                    ini.DataProc(type, section, $"{i}_ZPos", ref zpos);
                    ini.DataProc(type, section, $"{i}_LineSpeed", ref linespeed);
                    ini.DataProc(type, section, $"{i}_ZSpeed", ref zspeed);
                    ini.DataProc(type, section, $"{i}_Shot", ref shot);
            ...
            {
                section = cDefString.Bonder2Line;
                int jetlinecount = JetLineData.Count;
                ini.DataProc(type, section, "JetLineDataCount", ref jetlinecount);
                int index = 0;
                foreach (JettingLineData jld in JetLineData)
                {
                    ini.DataProc(type, section, $"{index}_XPos", ref jld.XPos);
                    ini.DataProc(type, section, $"{index}_YPos", ref jld.YPos);
                    ini.DataProc(type, section, $"{index}_ZPos", ref jld.ZPos);
                    ini.DataProc(type, section, $"{index}_LineSpeed", ref jld.LineSpeed);
                    ini.DataProc(type, section, $"{index}_ZSpeed", ref jld.ZSpeed);
                    ini.DataProc(type, section, $"{index}_Shot", ref jld.Shot);
                    index++;
                }
            }
        }
    }
    public class WorkingBonder2JetPTLine : BaseData
    {
        private string section;
        ...
        public WorkingBonder2JetPTLine(string name) : base(cDefString.Bonder2Pattern)
        {
        ...
        public override void DataProc(fpIni.IniType type)
        {
            ...
            {
                //PatternLine
                section = cDefString.Bonder2Pattern;
                int xpos = 0;
                int ypos = 0;
                int zpos = 0;
                int linespeed = 0;
    ...
    public class WorkingBonder2JetPTArc : BaseData
    {
        private string section;
        ...
        public WorkingBonder2JetPTArc(string name) : base(cDefString.Bonder2Arc)
        {
    ...
    public class WorkingBonder2 : BaseData
    {
        private string section;
        ...
        public WorkingBonder2(string name) : base(cDefString.Bonder2)
        {
        ...
        public override void DataProc(fpIni.IniType type)
        {
            ...
            ini.DataProc(type, section, "Bonder2GrabDelay", ref Bonder2GrabDelay);
    ...
    public class WorkingTeachingBonder2 : BaseData
    {
        private string section;
        ...
        public int TipCleanCntX = 0;
        public int TipCleanCntY = 0;

        public int TipCleanPitchX = 0;
        ...
        public WorkingTeachingBonder2(string name) : base(cDefString.Bonder2)
        {
            Clear();
         //   Load();
        }
        ...
        public void Clear()
        {
            ...
            CleanPositionX = 0;              //Head Clean Position   //Position 생성, Next 클리어 작업
            CleanPositionY = 0;              //Head Clean Position   
            CleanPositionZ = 0;              //Head Clean Position
        ...
        public override void DataProc(fpIni.IniType type)
        {
            ...
            ini.DataProc(type, section, "TipCleanCntX", ref TipCleanCntX);
            ini.DataProc(type, section, "TipCleanCntY", ref TipCleanCntY);
            ini.DataProc(type, section, "TipCleanPitchX", ref TipCleanPitchX);
